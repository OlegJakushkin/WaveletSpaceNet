"""Train WaveletSpaceNet v2 on randomised DIODE fly-throughs.
...
context), the camera pose relative to the context and a multi-level wavelet mesh-plane
(per-pixel depth).  Model selection is by held-out **symmetric** chamfer(m); per-epoch
render panels are written to ``renders/``.

  python train.py --smoke                       # tiny CPU/GPU sanity run (synthetic if no DIODE)
  python train.py --epochs 30 --batch 8         # full run (GPU)
  docker compose run --rm train --epochs 30     # in the GPU container
"""
...
def main():
    ...
    ap.add_argument("--warmup", type=int, default=800, help="linear LR warmup steps")
    ap.add_argument("--min-lr-frac", type=float, default=0.03, help="cosine floor as a fraction of --lr")
    ap.add_argument("--img-hw", type=int, default=512)
    ap.add_argument("--plane-res", type=int, default=256)
    ap.add_argument("--wave-levels", type=int, default=3, help="Haar octaves the decoder emits")
    ap.add_argument("--win", type=int, default=5, help="decoder local-attention window (win*win neighbours)")
    ...
    ap.add_argument("--fpn-dim", type=int, default=128)
    ...
    ap.add_argument("--max-scene-pts", type=int, default=200000, help="points kept per scene cloud")
    ap.add_argument("--splat-radius", type=int, default=2, help="INPUT render splat radius (GT is resolution-aware)")
    ...
    if a.smoke:
        a.epochs, a.batch, a.img_hw, a.plane_res = 2, 4, 64, 32
        a.d, a.M, a.L, a.heads, a.cap, a.workers = 96, 64, 3, 4, 24, 0
        a.wave_levels, a.warmup, a.device = 2, 10, "cpu"
        a.max_scene_pts = 40000
    dev = a.device
    ...
    # ---- model ------------------------------------------------------------
    torch.manual_seed(0)
    net = WaveletSpaceNet(d=a.d, M=a.M, L=a.L, heads=a.heads, plane_res=a.plane_res,
                          wave_levels=a.wave_levels, win=a.win, n_ctx=a.ctx_max,
                          img_size=a.img_hw, fpn_dim=a.fpn_dim).to(dev)
    img_p = net.img_tok.count_params() if hasattr(net.img_tok, "count_params") else \
        sum(p.numel() for p in net.img_tok.parameters())
    print(f"WaveletSpaceNet {net.count_params():,} params (image-embed {img_p:,}) | "
          f"img tokens {net.img_tok.n_tokens()} | plane_res {a.plane_res} | amp {use_amp} | device {dev}",
          flush=True)
    ...
    steps_per_epoch = max(1, len(ld))
    total_steps = max(1, a.epochs * steps_per_epoch)
        ...
        for batch in ld:
            ...
            if nb and nb % 25 == 0:
                ...
                print(f"  ep{ep+1} step{nb} loss {run['loss']/nb:.4f} depth {run['depth']/nb:.3f} "
                      f"grad {run['grad']/nb:.3f} rot {run['rot']/nb:.3f} trans {run['trans']/nb:.3f} "
                      f"lr {lr_at(gstep):.1e} {gpu}| {time.time()-t0:.0f}s", flush=True)
        val = validate()
