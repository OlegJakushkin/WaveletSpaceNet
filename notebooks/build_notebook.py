...
CELLS = [
    ...
    md("## 4 · Get DIODE: archive on Drive → extract to local (mask-free, fits 200 GB)",
       "",
       ...
       "",
       "The full train set extracted with masks is ~178 GB and overruns a 200 GB Colab disk, so the",
       "`*_depth_mask.npy` files are **skipped** — they are ~⅓ of the data and the loader derives the mask",
       "from `depth > 0` (99.97 % identical to the official mask).  That brings the footprint to ~95 GB.",
       "A `.train_extracted` sentinel marks success so re-runs don't re-extract; a failed partial extraction",
       "is cleaned first.  Set `FULL_TRAIN = False` for the small (2.8 GB) **val** split."),
    code("FULL_TRAIN = True   # True = complete DIODE train set (~87 GB). False = val split (~2.8 GB).",
         ...
         "",
         "def extract_no_masks(arc, dest, every=5000):",
         "    # stream the archive (low memory) and extract depth + rgb, SKIP *_depth_mask.npy",
         "    n = 0",
         ...
         "            n += 1",
         "            if n % every == 0: print(f'  extracted {n} files...', flush=True)",
         "    return n",
         "",
         "if FULL_TRAIN:",
         ...
         "        print('extracting from Drive -> data/diode (masks skipped; reads ~87 GB, takes a while)...')",
         "        n = extract_no_masks(arc, 'data/diode')",
         "        open(flag, 'w').write('ok')",
         "        print('extracted', n, 'files (depth + rgb)')",
         "    else:",
         "        print('already extracted (delete', flag, 'to redo)')",
         "else:",
