...
CELLS = [
    ...
    md("## 4 · Get DIODE: archive on Drive → extract to local (mask-free, capped for prototyping)",
       "",
       ...
       "from `depth > 0` (99.97 % identical to the official mask); (2) `MAX_VIEWS` **caps** how many views",
       "are extracted (stream stops early, so it only reads the first part of the 87 GB archive — much",
       "faster).  `MAX_VIEWS = 10000` (~35 GB, a few minutes) is a good prototyping default; set it to",
       "`None` for the full ~25 k-view set.  Re-extract with a different cap by deleting `.train_extracted`.",
       "",
       "Note: the archive is ordered indoors→outdoor, so the first 10 k views are mostly indoor scenes."),
    code("FULL_TRAIN = True       # True = DIODE train set. False = val split (~2.8 GB).",
         "MAX_VIEWS = 10000       # prototype cap on extracted views (None = all ~25k)",
         ...
         "",
         "def extract_no_masks(arc, dest, max_views=None, every=2000):",
         "    # stream the archive (low memory); extract depth + rgb, SKIP masks, stop after max_views",
         "    nv = 0",
         ...
         "                nv += 1",
         "                if nv % every == 0: print(f'  {nv} views...', flush=True)",
         "                if max_views and nv >= max_views:",
         "                    print(f'  reached cap of {max_views} views — stopping early.'); break",
         "    return nv",
         "",
         "if FULL_TRAIN:",
         ...
         "        print(f'extracting from Drive -> data/diode (masks skipped, cap={MAX_VIEWS})...')",
         "        nv = extract_no_masks(arc, 'data/diode', max_views=MAX_VIEWS)",
         "        open(flag, 'w').write(str(nv))",
         "        print('extracted', nv, 'views (depth + rgb)')",
         "    else:",
         ...
         "        extract_no_masks(arc, 'data/diode')",
